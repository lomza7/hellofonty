-- Retry batch 8: video bucket, share token, leases, booking_id on leases, feature_carousel_images
...
DROP POLICY IF EXISTS "Landlords can update own draft leases" ON leases;
...
DROP POLICY IF EXISTS "Tenants can sign their leases" ON leases;
...
DROP POLICY IF EXISTS "Landlords can delete draft leases" ON leases;
